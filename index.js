...
app.get("/api/vizinhos/:cidade/:estado", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Falha ao buscar vizinhos: ${error.message}` });
  }
});
...
app.delete("/api/excluir/:rowIndex", async (req, res) => {
  try {
    ...
    if (rowToDelete) {
      ...
      res.status(404).json({
        error: `Registro não encontrado para rowIndex: ${rowIndexToDelete}`,
      });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: `Falha ao excluir o registro: ${error.message}` });
  }
});
...
app.listen(PORT, () => {
  ...
  if (!OPENWEATHER_API_KEY) {
    console.warn(
      "AVISO: Chave da API OpenWeatherMap não encontrada. A busca por cidades vizinhas não funcionará."
    );
  }
});
