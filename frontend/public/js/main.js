...
const methods = {
  aplicarFiltrosErenderizar: () => {
    ...
    let dadosProcessados = appState.dadosCompletos.filter((item) => {
      ...
      const valorItem =
        parseFloat(String(item.valor_pela_sns).replace(",", ".")) || 0;

      return (
        (cidade ? cidadeItem.includes(cidade) : true) &&
        (nome
          ? medicoItem.includes(nome) || clinicaItem.includes(nome)
          : true) &&
    ...
    if (ordenacao === "preco-menor") {
      dadosProcessados.sort(
        (a, b) =>
          (parseFloat(String(a.valor_pela_sns).replace(",", ".")) || 0) -
          (parseFloat(String(b.valor_pela_sns).replace(",", ".")) || 0)
      );
  ...
  refreshData: async () => {
    try {
      ...
      DOMElements.resultadosContainer.innerHTML =
        "<p>Não foi possível carregar os dados. Verifique a conexão com o servidor.</p>";
    }
  },
};
